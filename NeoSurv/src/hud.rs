...
pub(crate) fn build_hud_meshes(
    ...
    if let Some(player) = world.local_player() {
        ...
        meshes.push(transform_overlay_mesh(
            ...
            "hud-health-shell-highlight-overlay",
            camera,
        ...
        for index in 0..4usize {
            ...
            meshes.push(transform_overlay_mesh(
                ...
                format!("hud-slot-shell-{index}-overlay"),
                camera,
                HUD_OFFSET,
            ));
            meshes.push(transform_overlay_mesh(
                ...
                format!("hud-slot-accent-{index}-overlay"),
                camera,
                HUD_OFFSET + Vec3::new(0.0, 0.0, 0.01),
            ));
            meshes.push(transform_overlay_mesh(
                ...
                format!("hud-slot-number-{index}-overlay"),
                camera,
            ...
            if count > 1 {
                ...
                meshes.push(transform_overlay_mesh(
                    ...
                    format!("hud-slot-count-bg-{index}-overlay"),
                    camera,
                    HUD_OFFSET + Vec3::new(0.0, 0.0, 0.015),
                ));
                meshes.push(transform_overlay_mesh(
                    ...
                    format!("hud-slot-count-{index}-overlay"),
                    camera,
                    HUD_OFFSET + Vec3::new(0.0, 0.0, 0.02),
                ));
            }
    ...
    meshes.extend(chat.build_overlay_meshes(camera));
    meshes
}
